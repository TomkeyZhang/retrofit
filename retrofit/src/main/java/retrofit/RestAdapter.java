 ...
 * <p>
 * API endpoints are defined as methods on an interface with annotations providing metadata about the form in which the
 * HTTP call should be made.
 * <p>
 * The relative path for a given method is obtained from an annotation on the method describing the request type. The
 * built-in methods are {@link retrofit.http.GET GET}, {@link retrofit.http.PUT PUT}, {@link retrofit.http.POST POST},
 * {@link retrofit.http.HEAD HEAD}, and {@link retrofit.http.DELETE DELETE}. You can define your own HTTP method by
 * creating an annotation that takes a {code String} value and itself is annotated with {@link retrofit.http.RestMethod
 * @RestMethod}.
 * <p>
 * Method parameters can be used to replace parts of the URL by annotating them with {@link retrofit.http.Path @Path}.
 * Replacement sections are denoted by an identifier surrounded by curly braces (e.g., "{foo}"). To add items to the
 * query string of a URL use {@link retrofit.http.Query @Query}. If the path or query element has already been URI
 * encoded use {@link retrofit.http.EncodedPath @EncodedPath} or {@link retrofit.http.EncodedQuery @EncodedQuery} to
 * prevent repeated encoding.
 * <p>
 * HTTP requests happen in one of two ways:
 * <ul>
 * <li>On the provided HTTP {@link Executor} with callbacks marshaled to the callback {@link Executor}. The last method
 * parameter should be of type {@link Callback}. The HTTP response will be converted to the callback's parameter type
 * using the specified {@link retrofit.converter.Converter Converter}. If the callback parameter type uses a wildcard,
 * the lower bound will be used as the conversion type.
 * <li>On the current thread returning the response or throwing a {@link RetrofitError}. The HTTP response will be
 * converted to the method's return type using the specified {@link retrofit.converter.Converter Converter}.
 * </ul>
 * <p>
 * The body of a request is denoted by the {@link retrofit.http.Body @Body} annotation. The object will be converted to
 * request representation by a call to {@link retrofit.converter.Converter#toBody(Object) toBody} on the supplied
 * {@link retrofit.converter.Converter Converter} for this instance. The body can also be a {@link TypedOutput} where it
 * will be used directly.
 * <p>
 * Alternative request body formats are supported by method annotations and corresponding parameter annotations:
 * <ul>
 * <li>{@link retrofit.http.FormUrlEncoded @FormUrlEncoded} - Form-encoded data with key-value pairs specified by the
 * {@link retrofit.http.Field @Field} parameter annotation.
 * <li>{@link retrofit.http.Multipart @Multipart} - RFC 2387-compliant multi-part data with parts specified by the
 * {@link retrofit.http.Part @Part} parameter annotation.
 * </ul>
 * <p>
 * Additional static headers can be added for an endpoint using the {@link retrofit.http.Headers @Headers} method
 * annotation. For per-request control over a header annotate a parameter with {@link Header @Header}.
 * <p>
 * For example:
 * 
 * <pre>
...
public class RestAdapter {
	...
	private RestAdapter(Endpoint server, Client.Provider clientProvider,
			Executor httpExecutor, Executor callbackExecutor,
			RequestInterceptor requestInterceptor, Converter converter,
			Profiler profiler, ErrorHandler errorHandler, Log log,
			LogLevel logLevel) {
		this.server = server;
	...
	public <T> T create(Class<T> service) {
		...
		return (T) Proxy.newProxyInstance(service.getClassLoader(),
				new Class<?>[] { service }, new RestHandler(
						getMethodInfoCache(service)));
	}

	Map<Method, RestMethodInfo> getMethodInfoCache(Class<?> service) {
		synchronized (serviceMethodInfoCache) {
			Map<Method, RestMethodInfo> methodInfoCache = serviceMethodInfoCache
					.get(service);
	...
	static RestMethodInfo getMethodInfo(Map<Method, RestMethodInfo> cache,
			Method method) {
		synchronized (cache) {
			...
			return Observable.create(new Observable.OnSubscribeFunc<Object>() {
				...
				public Subscription onSubscribe(
					...
					try {
						...
						// This is from the Callable. It shouldn't actually
						// throw.
...
//			final RequestInterceptorTape interceptorTape = new RequestInterceptorTape();
			...
			if (methodInfo.isObservable) {
				return rxSupport
						.createRequestObservable(new Callable<ResponseWrapper>() {
							@Override
							public ResponseWrapper call() throws Exception {
								return (ResponseWrapper) invokeRequest(
										requestInterceptor, methodInfo, args);
							}
						});
			}
		...
		private Object invokeRequest(RequestInterceptor requestInterceptor,
			...
			String url = serverUrl; // Keep some url in case RequestBuilder
									// throws an exception.
			try {
...
//				methodInfo.setBody(getBody(requestBuilder.build()));
//				if (requestInterceptor instanceof RequestInterceptorTape) {
//					RequestInterceptorTape interceptorTape = (RequestInterceptorTape) requestInterceptor;
...
//				}
				...
				if (!methodInfo.isSynchronous) {
					...
					Thread.currentThread().setName(
							THREAD_PREFIX + url.substring(serverUrl.length()));
				}
				...
				long elapsedTime = TimeUnit.NANOSECONDS.toMillis(System
						.nanoTime() - start);
				...
				if (profiler != null) {
					...
					profiler.afterCall(requestInfo, elapsedTime, statusCode,
							profilerObject);
				}
				...
				if (statusCode >= 200 && statusCode < 300) { // 2XX ==
					...
					if (type.equals(Response.class)) {
						// Read the entire stream and replace with one backed by
						// a byte[]
					...
					try {
						...
						throw RetrofitError.conversionError(url, response,
								converter, type, e);
					}
				}
	...
	Request logAndReplaceRequest(String name, Request request)
			throws IOException {
		log.log(String.format("---> %s %s %s", name, request.getMethod(),
				request.getUrl()));
	 ...
	 */
	private Response logAndReplaceResponse(String url, Response response,
			long elapsedTime) throws IOException {
		log.log(String.format("<--- HTTP %s %s (%sms)", response.getStatus(),
				url, elapsedTime));
	...
	private static Profiler.RequestInformation getRequestInfo(String serverUrl,
		...
		return new Profiler.RequestInformation(methodDetails.requestMethod,
				serverUrl, methodDetails.requestUrl, contentLength, contentType);
	}
	 ...
	 * </ul>
	 * <p>
	 * If you are using asynchronous execution (i.e., with {@link Callback
	 * Callbacks}) the following is also required:
	 * <ul>
	 * <li>
	...
	public static class Builder {
		 ...
		 * API server base URL.
		 * 
		 * @deprecated Use {@link #setEndpoint(String)} or
		 *             {@link #setEndpoint(Endpoint)}. This method will be
		 ...
		 */
		public Builder setExecutors(Executor httpExecutor,
